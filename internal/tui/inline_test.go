...
func TestExpandInline_ExpandsDirItems(t *testing.T) {
	...
	// "New window" built-in + "Browse" from config
	if len(result) != 2 {
		t.Fatalf("got %d items, want 2", len(result))
	}
...
func TestExpandInline_PreservesLaunchMetadata(t *testing.T) {
	...
	pi := result[1]
	if pi.Value != "Pi" {
		t.Fatalf("result[1].Value = %q, want Pi", pi.Value)
	}
	if pi.MatchType != "dir" {
...
func TestExpandInline_MultipleParents(t *testing.T) {
	...
	// Each dir gets the built-in window action.
	if len(result) != 2 {
		t.Fatalf("got %d items, want 2", len(result))
	}
	wantDisplays := []string{
	...
	if result[0].Data["path"] != "/a" {
		t.Errorf("first parent path = %q, want /a", result[0].Data["path"])
	}
	if result[1].Data["path"] != "/b" {
		t.Errorf("second parent path = %q, want /b", result[1].Data["path"])
	}
}
