...
var exitCodesTopic = &cobra.Command{
  ...
  1   cmdk error. An invalid config file passed via --config, an
      ...
      problems discovered in the default config path are surfaced inside
      the TUI rather than causing a non-zero exit. Logging setup failures
      produce a stderr warning and do not cause exit 1.
